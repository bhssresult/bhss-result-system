...
A second, **independent** automation lives in `group-sync.gs` (repo root) — **not** part of the Web App `Code.gs`. It mirrors ranges of the `HS_Teachers` tab into **Google Groups** via the **AdminDirectory** advanced service, driven by a `GROUP_MAPPINGS` table (range → group email): `F2:G17` → the master group `bhss-hs-teachers@baptisthss.in`, plus one per-subject group per row (`F2:G2` … `F17:G17`). On edit, `onRangeEdit` syncs every mapping whose range overlaps the edit (via `rangesOverlap`), so editing a teacher row updates both its subject group and the master. Ranges are **fixed A1 strings** (rows 2–17); extend `GROUP_MAPPINGS` if teachers are added past row 17. Any `@`-bearing cell in a mapped range counts as a member; managers/owners and `PROTECTED_EMAILS` are never auto-removed. Pasted into the workbook's Apps Script (own file or own project), it requires a **Workspace admin** + the **Admin SDK API** enabled, and is set up by running `manualSync()` then `installTrigger()`. It shares no code with `Code.gs` and needs no Web App redeploy — both simply read `HS_Teachers` and write to different destinations (`Users` sheet vs. the Groups).
